...
--- 
## 10. 아쉬웠던 점 및 배운점 
학교에서 배웠던 데이터 분석의 기본 내용들을 바탕으로 처음 제대로 된 실습을 해봤습니다. 어떤 걸 주제로 삼아서 분석을 할 지 막막했었는데 팀원분들과 서로 의지하면서 수행한 덕분에 잘 마무리할 수 있었습니. 다만, '재정데이터 분석'인데 재정 데이터를 적극 활용하지 못한 부분은 아쉬웠습니다. 다음에 기회가 된다면 재정데이터를 적극 활용해서 저희 주제에 대한 결론 부분을 좀 더 강화해보고 싶습니다. 처음한 데이터 분석 프로젝트여서 어떤 식으로 분석 과정이 이루어지는 지 배울 수 있었고 이번 기회를 통해 데이터 분석에 더 많은 관심을 가지게 되었습니다. 본 프로젝트를 통해 데이터 분석이 문제 정의 → 가설 설정 → 분석 → 해석 → 정책 제안으로 이어지는 하나의 사고 과정임을 체감할 수 있었습니다.
